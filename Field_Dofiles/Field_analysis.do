...
* change $zamb path to run in self-contained way 
	global zamb "/Users/aditichugh/Desktop/zambia"
...
* Define list of outcome variables (for which ttest by gender is performed)
...
* Loop through each outcome variable and run t-tests by gender
	foreach var in `outc' {
	estpost ttest `var', by(gender) 
	est store `var'_1
	
	* Export results as a TeX file
	estout `var'_1 ///
...
* export number of observations as tex file
...
* Loop to run a linear regression for both dependent variables- with and without the covariates
foreach var in `depen' {
	...
	* Export as separate tex file
			...
			using "$lat\reg_`var'_all.tex", label replace /// *specifies the name and location of output file
			cells(b(star fmt(3)) se(par fmt(3))) style(tex) /// *specifies the format for the coefficient estimates and standard errors
			mlabels(, none) collabels(, none) varlabels(_cons cons) ///
			eqlabels(, none) starlevels(* 0.10 ** 0.05 *** 0.01) ///
			keep(`cov_all_hh' _cons) /// *which variables to include in the table
